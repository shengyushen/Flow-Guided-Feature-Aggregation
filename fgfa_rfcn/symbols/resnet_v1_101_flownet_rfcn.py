...
class resnet_v1_101_flownet_rfcn(Symbol):
    ...
    def get_train_symbol(self, cfg):
        ...
        # resnet and flownet is not sequential, they are parallel
        # and resnet is not linear to the depth, while the flownet is linear
        ...
        if cfg.TRAIN.CXX_PROPOSAL:
            ...
            # ROI proposal target
        #_,oshape,_=rois.infer_shape(data=(1, 3, 300, 300),data_bef=(1,3,300,300),data_aft=(1,3,300,300))
        #print("rois shape")
        #print(str(oshape))
        #_,oshape,_=gt_boxes.infer_shape(data=(1, 3, 300, 300),data_bef=(1,3,300,300),data_aft=(1,3,300,300))
    ...
    def get_train_symbol_5(self, cfg):
        # config alias for convenient
        num_classes = cfg.dataset.NUM_CLASSES
        num_reg_classes = (2 if cfg.CLASS_AGNOSTIC else num_classes)
        num_anchors = cfg.network.NUM_ANCHORS
        ...
        data = mx.sym.Variable(name="data")
        data_bef1 = mx.sym.Variable(name="data_bef1")
        data_bef2 = mx.sym.Variable(name="data_bef2")
        data_aft1 = mx.sym.Variable(name="data_aft1")
        data_aft2 = mx.sym.Variable(name="data_aft2")
        im_info = mx.sym.Variable(name="im_info")
        gt_boxes = mx.sym.Variable(name="gt_boxes")
        rpn_label = mx.sym.Variable(name='label')
        rpn_bbox_target = mx.sym.Variable(name='bbox_target')
        ...
        # pass through ResNet
        # dim=0 means the batch dimension
        concat_data = mx.symbol.Concat(*[data, data_bef1,data_bef2,data_aft1,data_aft2], dim=0)
        conv_feat = self.get_resnet_v1(concat_data)

        # pass through FlowNet
        # resnet and flownet is not sequential, they are parallel
        # and both are pass through the resnet and flownet as data parallel
        concat_flow_data_bef1 = mx.symbol.Concat(data / 255.0, data_bef1 / 255.0, dim=1)
        concat_flow_data_bef2 = mx.symbol.Concat(data / 255.0, data_bef2 / 255.0, dim=1)
        concat_flow_data_aft1 = mx.symbol.Concat(data / 255.0, data_aft1 / 255.0, dim=1)
        concat_flow_data_aft2 = mx.symbol.Concat(data / 255.0, data_aft2 / 255.0, dim=1)
        # dim=0 means the batch dimension
        concat_flow_data = mx.symbol.Concat(*[concat_flow_data_bef1, concat_flow_data_bef2,concat_flow_data_aft1,concat_flow_data_aft2], dim=0)
        flow = self.get_flownet(concat_flow_data)

        flow = mx.sym.SliceChannel(flow, axis=0, num_outputs=4)

        conv_feat = mx.sym.SliceChannel(conv_feat, axis=0, num_outputs=5)

        # warping
        flow_grid_bef1 = mx.sym.GridGenerator(data=flow[0], transform_type='warp', name='flow_grid_bef1')
        flow_grid_bef2 = mx.sym.GridGenerator(data=flow[1], transform_type='warp', name='flow_grid_bef2')
        flow_grid_aft1 = mx.sym.GridGenerator(data=flow[2], transform_type='warp', name='flow_grid_aft1')
        flow_grid_aft2 = mx.sym.GridGenerator(data=flow[3], transform_type='warp', name='flow_grid_aft2')

        warp_conv_feat_bef1 = mx.sym.BilinearSampler(data=conv_feat[1], grid=flow_grid_bef1, name='warping_feat_bef1')
        warp_conv_feat_bef2 = mx.sym.BilinearSampler(data=conv_feat[2], grid=flow_grid_bef2, name='warping_feat_bef2')
        warp_conv_feat_aft1 = mx.sym.BilinearSampler(data=conv_feat[3], grid=flow_grid_aft1, name='warping_feat_aft1')
        warp_conv_feat_aft2 = mx.sym.BilinearSampler(data=conv_feat[4], grid=flow_grid_aft2, name='warping_feat_aft2')

        # pass through EmbedNet
        # this is the place to warp resnet and flownet together
        concat_embed_data = mx.symbol.Concat(*[conv_feat[0], warp_conv_feat_bef1, warp_conv_feat_bef2,warp_conv_feat_aft1,warp_conv_feat_aft2], dim=0)
        embed_output = self.get_embednet(concat_embed_data)
        embed_output = mx.sym.SliceChannel(embed_output, axis=0, num_outputs=5)

        unnormalize_weight_bef1 = self.compute_weight(embed_output[1], embed_output[0])
        unnormalize_weight_bef2 = self.compute_weight(embed_output[2], embed_output[0])
        unnormalize_weight_aft1 = self.compute_weight(embed_output[3], embed_output[0])
        unnormalize_weight_aft2 = self.compute_weight(embed_output[4], embed_output[0])
        unnormalize_weights = mx.symbol.Concat(*[unnormalize_weight_bef1, unnormalize_weight_bef2, unnormalize_weight_aft1,unnormalize_weight_aft2], dim=0)

        weights = mx.symbol.softmax(data=unnormalize_weights, axis=0)
        weights = mx.sym.SliceChannel(weights, axis=0, num_outputs=4)

        # tile the channel dim of weights
        weight_bef1 = mx.symbol.tile(data=weights[0], reps=(1, 1024, 1, 1))
        weight_bef2 = mx.symbol.tile(data=weights[1], reps=(1, 1024, 1, 1))
        weight_aft1 = mx.symbol.tile(data=weights[2], reps=(1, 1024, 1, 1))
        weight_aft2 = mx.symbol.tile(data=weights[3], reps=(1, 1024, 1, 1))
        select_conv_feat = weight_bef1 * warp_conv_feat_bef1 + weight_bef2 * warp_conv_feat_bef2 + weight_aft1*warp_conv_feat_aft1 + weight_aft2*warp_conv_feat_aft2
        # the end of input data difference
        conv_feats = mx.sym.SliceChannel(select_conv_feat, axis=1, num_outputs=2)
    ...
    def get_train_symbol_11(self, cfg):
        # config alias for convenient
        num_classes = cfg.dataset.NUM_CLASSES
        num_reg_classes = (2 if cfg.CLASS_AGNOSTIC else num_classes)
        num_anchors = cfg.network.NUM_ANCHORS
        ...
        data_bef1 = mx.sym.Variable(name="data_bef1")
        data_bef2 = mx.sym.Variable(name="data_bef2")
        data_bef3 = mx.sym.Variable(name="data_bef3")
        data_bef4 = mx.sym.Variable(name="data_bef4")
        data_bef5 = mx.sym.Variable(name="data_bef5")
        data_aft1 = mx.sym.Variable(name="data_aft1")
        data_aft2 = mx.sym.Variable(name="data_aft2")
        data_aft3 = mx.sym.Variable(name="data_aft3")
        data_aft4 = mx.sym.Variable(name="data_aft4")
        data_aft5 = mx.sym.Variable(name="data_aft5")
        im_info = mx.sym.Variable(name="im_info")
        gt_boxes = mx.sym.Variable(name="gt_boxes")
        rpn_label = mx.sym.Variable(name='label')
        rpn_bbox_target = mx.sym.Variable(name='bbox_target')
        ...
        # pass through ResNet
        # dim=0 means the batch dimension
        concat_data = mx.symbol.Concat(*[data, data_bef1,data_bef2,data_bef3,data_bef4,data_bef5,data_aft1,data_aft2,data_aft3,data_aft4,data_aft5], dim=0)
        conv_feat = self.get_resnet_v1(concat_data)
        ...
        concat_flow_data_bef1 = mx.symbol.Concat(data / 255.0, data_bef1 / 255.0, dim=1)
        concat_flow_data_bef2 = mx.symbol.Concat(data / 255.0, data_bef2 / 255.0, dim=1)
        concat_flow_data_bef3 = mx.symbol.Concat(data / 255.0, data_bef3 / 255.0, dim=1)
        concat_flow_data_bef4 = mx.symbol.Concat(data / 255.0, data_bef4 / 255.0, dim=1)
        concat_flow_data_bef5 = mx.symbol.Concat(data / 255.0, data_bef5 / 255.0, dim=1)
        concat_flow_data_aft1 = mx.symbol.Concat(data / 255.0, data_aft1 / 255.0, dim=1)
        concat_flow_data_aft2 = mx.symbol.Concat(data / 255.0, data_aft2 / 255.0, dim=1)
        concat_flow_data_aft3 = mx.symbol.Concat(data / 255.0, data_aft3 / 255.0, dim=1)
        concat_flow_data_aft4 = mx.symbol.Concat(data / 255.0, data_aft4 / 255.0, dim=1)
        concat_flow_data_aft5 = mx.symbol.Concat(data / 255.0, data_aft5 / 255.0, dim=1)
        # dim=0 means the batch dimension
        concat_flow_data = mx.symbol.Concat(*[concat_flow_data_bef1, concat_flow_data_bef2,concat_flow_data_bef3,concat_flow_data_bef4,concat_flow_data_bef5,concat_flow_data_aft1,concat_flow_data_aft2,concat_flow_data_aft3,concat_flow_data_aft4,concat_flow_data_aft5], dim=0)
        flow = self.get_flownet(concat_flow_data)

        flow = mx.sym.SliceChannel(flow, axis=0, num_outputs=10)

        conv_feat = mx.sym.SliceChannel(conv_feat, axis=0, num_outputs=11)

        # warping
        flow_grid_bef1 = mx.sym.GridGenerator(data=flow[0], transform_type='warp', name='flow_grid_bef1')
        flow_grid_bef2 = mx.sym.GridGenerator(data=flow[1], transform_type='warp', name='flow_grid_bef2')
        flow_grid_bef3 = mx.sym.GridGenerator(data=flow[2], transform_type='warp', name='flow_grid_bef3')
        flow_grid_bef4 = mx.sym.GridGenerator(data=flow[3], transform_type='warp', name='flow_grid_bef4')
        flow_grid_bef5 = mx.sym.GridGenerator(data=flow[4], transform_type='warp', name='flow_grid_bef5')
        flow_grid_aft1 = mx.sym.GridGenerator(data=flow[5], transform_type='warp', name='flow_grid_aft1')
        flow_grid_aft2 = mx.sym.GridGenerator(data=flow[6], transform_type='warp', name='flow_grid_aft2')
        flow_grid_aft3 = mx.sym.GridGenerator(data=flow[7], transform_type='warp', name='flow_grid_aft3')
        flow_grid_aft4 = mx.sym.GridGenerator(data=flow[8], transform_type='warp', name='flow_grid_aft4')
        flow_grid_aft5 = mx.sym.GridGenerator(data=flow[9], transform_type='warp', name='flow_grid_aft5')

        warp_conv_feat_bef1 = mx.sym.BilinearSampler(data=conv_feat[1], grid=flow_grid_bef1, name='warping_feat_bef1')
        warp_conv_feat_bef2 = mx.sym.BilinearSampler(data=conv_feat[2], grid=flow_grid_bef2, name='warping_feat_bef2')
        warp_conv_feat_bef3 = mx.sym.BilinearSampler(data=conv_feat[3], grid=flow_grid_bef3, name='warping_feat_bef3')
        warp_conv_feat_bef4 = mx.sym.BilinearSampler(data=conv_feat[4], grid=flow_grid_bef4, name='warping_feat_bef4')
        warp_conv_feat_bef5 = mx.sym.BilinearSampler(data=conv_feat[5], grid=flow_grid_bef5, name='warping_feat_bef5')

        warp_conv_feat_aft1 = mx.sym.BilinearSampler(data=conv_feat[6], grid=flow_grid_aft1, name='warping_feat_aft1')
        warp_conv_feat_aft2 = mx.sym.BilinearSampler(data=conv_feat[7], grid=flow_grid_aft2, name='warping_feat_aft2')
        warp_conv_feat_aft3 = mx.sym.BilinearSampler(data=conv_feat[8], grid=flow_grid_aft3, name='warping_feat_aft3')
        warp_conv_feat_aft4 = mx.sym.BilinearSampler(data=conv_feat[9], grid=flow_grid_aft4, name='warping_feat_aft4')
        warp_conv_feat_aft5 = mx.sym.BilinearSampler(data=conv_feat[10], grid=flow_grid_aft5, name='warping_feat_aft5')

        # pass through EmbedNet
        # this is the place to warp resnet and flownet together
        concat_embed_data = mx.symbol.Concat(*[conv_feat[0], warp_conv_feat_bef1, warp_conv_feat_bef2,warp_conv_feat_bef3,warp_conv_feat_bef4,warp_conv_feat_bef5,warp_conv_feat_aft1,warp_conv_feat_aft2,warp_conv_feat_aft3,warp_conv_feat_aft4,warp_conv_feat_aft5], dim=0)
        embed_output = self.get_embednet(concat_embed_data)
        embed_output = mx.sym.SliceChannel(embed_output, axis=0, num_outputs=11)

        unnormalize_weight_bef1 = self.compute_weight(embed_output[1], embed_output[0])
        unnormalize_weight_bef2 = self.compute_weight(embed_output[2], embed_output[0])
        unnormalize_weight_bef3 = self.compute_weight(embed_output[3], embed_output[0])
        unnormalize_weight_bef4 = self.compute_weight(embed_output[4], embed_output[0])
        unnormalize_weight_bef5 = self.compute_weight(embed_output[5], embed_output[0])

        unnormalize_weight_aft1 = self.compute_weight(embed_output[6], embed_output[0])
        unnormalize_weight_aft2 = self.compute_weight(embed_output[7], embed_output[0])
        unnormalize_weight_aft3 = self.compute_weight(embed_output[8], embed_output[0])
        unnormalize_weight_aft4 = self.compute_weight(embed_output[9], embed_output[0])
        unnormalize_weight_aft5 = self.compute_weight(embed_output[10], embed_output[0])
        unnormalize_weights = mx.symbol.Concat(*[unnormalize_weight_bef1, unnormalize_weight_bef2, unnormalize_weight_bef3, unnormalize_weight_bef4, unnormalize_weight_bef5, unnormalize_weight_aft1,unnormalize_weight_aft2,unnormalize_weight_aft3,unnormalize_weight_aft4,unnormalize_weight_aft5], dim=0)

        weights = mx.symbol.softmax(data=unnormalize_weights, axis=0)
        weights = mx.sym.SliceChannel(weights, axis=0, num_outputs=10)

        # tile the channel dim of weights
        weight_bef1 = mx.symbol.tile(data=weights[0], reps=(1, 1024, 1, 1))
        weight_bef2 = mx.symbol.tile(data=weights[1], reps=(1, 1024, 1, 1))
        weight_bef3 = mx.symbol.tile(data=weights[2], reps=(1, 1024, 1, 1))
        weight_bef4 = mx.symbol.tile(data=weights[3], reps=(1, 1024, 1, 1))
        weight_bef5 = mx.symbol.tile(data=weights[4], reps=(1, 1024, 1, 1))

        weight_aft1 = mx.symbol.tile(data=weights[5], reps=(1, 1024, 1, 1))
        weight_aft2 = mx.symbol.tile(data=weights[6], reps=(1, 1024, 1, 1))
        weight_aft3 = mx.symbol.tile(data=weights[7], reps=(1, 1024, 1, 1))
        weight_aft4 = mx.symbol.tile(data=weights[8], reps=(1, 1024, 1, 1))
        weight_aft5 = mx.symbol.tile(data=weights[9], reps=(1, 1024, 1, 1))
        select_conv_feat = weight_bef1 * warp_conv_feat_bef1 + weight_bef2 * warp_conv_feat_bef2+ weight_bef3 * warp_conv_feat_bef3+ weight_bef4 * warp_conv_feat_bef4+ weight_bef5 * warp_conv_feat_bef5 + weight_aft1*warp_conv_feat_aft1 + weight_aft2*warp_conv_feat_aft2+ weight_aft3*warp_conv_feat_aft3+ weight_aft4*warp_conv_feat_aft4+ weight_aft5*warp_conv_feat_aft5
        # the end of input data difference
        conv_feats = mx.sym.SliceChannel(select_conv_feat, axis=1, num_outputs=2)
